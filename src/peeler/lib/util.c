...
// util.c
// Shared utility implementations: CRC routines and growable output buffers.
...
// Default initial capacity when the caller requests zero.
#define GROW_DEFAULT_CAP 4096

// ============================================================================
// CRC-16/CCITT — architecture.md § "CRC Routines"
// ============================================================================

// Pre-computed lookup table for CRC-16/CCITT (polynomial 0x1021, MSB-first).
// Generated with: for each byte b in 0..255, shift through 8 bits, XOR with
// 0x1021 whenever the high bit is set.
...
// Update a running CRC-16/CCITT with additional data bytes.
uint16_t crc16_ccitt_update(uint16_t crc, const uint8_t *data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        // XOR byte into high byte of CRC, look up in table
...
// Compute CRC-16/CCITT over a complete buffer (init value 0).
uint16_t crc16_ccitt(const uint8_t *data, size_t len) {
    return crc16_ccitt_update(0, data, len);
}
...
// Ensure the growable buffer has room for at least `extra` more bytes.
static void grow_ensure(grow_buf_t *g, size_t extra, decode_ctx_t *ctx) {
    size_t needed = g->len + extra;
    if (needed <= g->cap) {
        return;
    }
    // Grow by at least 1.5x the current capacity
    size_t new_cap = g->cap + g->cap / 2;
    if (new_cap < needed) {
        new_cap = needed;
    }
...
// Append n bytes from src to the growable buffer.
...
// Append a single byte to the growable buffer.
...
peel_buf_t grow_finish(grow_buf_t *g) {
    ...
    // Zero the grow_buf so it cannot be reused
    memset(g, 0, sizeof(*g));
    return result;
}
