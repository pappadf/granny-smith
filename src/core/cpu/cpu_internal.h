...
// Set CHK/CHK2 undefined flags for 68020/68030 (matches real hardware behavior).
// src=bound, dst=Dn value, is_long=true for .L, false for .W.
static inline void chk_undefined_flags_030(cpu_t *restrict cpu, int32_t src, int32_t dst, bool is_long) {
    // Clear C, Z, N, V
    cpu->carry = 0;
    cpu->zero = 0;
    cpu->negative = 0;
    cpu->overflow = 0;
    // Z set if register value is zero
    if (dst == 0)
        cpu->zero = 1;
    // N always reflects sign of register value
    cpu->negative = (dst < 0);
    // V and C only set in the exception case (out of bounds)
    if (dst < 0 || dst > src) {
        if (is_long) {
            int flgs = dst < 0;
            int flgo = src < 0;
            int32_t val = src - dst;
            int flgn = val < 0;
            cpu->overflow = (flgs ^ flgo) & (flgn ^ flgo);
        } else {
            int flgs = ((int16_t)dst) < 0;
            int flgo = ((int16_t)src) < 0;
            int16_t val = (int16_t)src - (int16_t)dst;
            int flgn = val < 0;
            cpu->overflow = (flgs ^ flgo) & (flgn ^ flgo);
        }
        // C flag depends on sign relationship
        if (dst < 0) {
            cpu->carry = (dst > src || src >= 0);
        } else {
            cpu->carry = (src >= 0);
        }
    }
...
// Set CHK2/CMP2 undefined flags (N, V) for 68020/68030.
// lower/upper are the signed bounds, val is the signed register value.
// Subtractions use int64_t to avoid signed 32-bit overflow (UB in C).
static inline void chk2_undefined_flags_030(cpu_t *restrict cpu, int32_t lower, int32_t upper, int32_t val) {
    cpu->negative = 0;
    cpu->overflow = 0;

    // Equal to a bound: N=0, V=0 (early exit)
    if (val == lower || val == upper)
        return;

    // Promote to 64-bit for overflow-safe subtraction
    int64_t lo = lower, hi = upper, v = val;
