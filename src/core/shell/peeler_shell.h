...
// Initializes and registers peeler shell commands
void peeler_shell_init(void);

// Extract a single archive (`path`) into `out_dir` (defaults to "."
// if NULL/empty). Used by the typed `peeler` root method and the
// legacy `peeler` shell command.  Returns 0 on success, non-zero on
// failure (file not found, format unsupported, write failure).
