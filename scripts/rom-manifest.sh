...
out.append("> the grammar in the emulator's `scripts/rom_naming.py`. The extension distinguishes CPU ROMs")
...
if dest == "-":
    ...
    with open(dest, "w", encoding="utf-8") as f:
        f.write(text)
    print(f"rom-manifest: wrote {len(cpu)} CPU ROM(s) + {len(vro)} vROM(s) + {len(pro)} PROM(s) to {dest}", file=sys.stderr)
PY
