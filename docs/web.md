...
The two callbacks are the template for any future C→JS event: install on `Module.*`, fire from C with `MAIN_THREAD_*_EM_ASM`. No exports, no SAB plumbing, no JS-side timers.
...
- `main.mjs` (generated by Emscripten) is dynamically imported with a cache-busting query parameter. `createModule` is invoked with `canvas`, `arguments`, `locateFile`, `print`/`printErr`, and the two state-push callbacks (`onRunStateChange`, `onPromptChange`).
...
The four request kinds all use the same slot (one in flight at a time, serialised by `cmdInFlight` in JS):

| `pending` | Operation | `path` / `args` carry | `output` carries |
|---|---|---|---|
| `1` | `gs_eval(path, args)` | path + JSON arg array | JSON-encoded `value_t` |
| `2` | `gs_inspect(path)` | path | JSON-encoded `value_t` |
| `3` | tab complete | line + cursor pos (decimal text in `args`) | JSON array of completion strings |
...
Two pieces of state are pushed via `Module.*` callbacks, set on the config object passed to `createModule`:
...
JS caches the values in module-locals (`isRunningUI`, `cachedPrompt`) and exposes them through `isRunning()` / `getRuntimePrompt()` for synchronous access from UI code.
