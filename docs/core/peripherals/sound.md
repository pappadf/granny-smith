...
once before committing); see `tests/integration/suite-plus/` (the `plus-beep` row).
...
Testing: headless golden-WAV capture tests (§4a) exercise the producer path deterministically; see `tests/integration/suite-plus/` (the `plus-beep` row).
