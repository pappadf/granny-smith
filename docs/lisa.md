...
| 2 | Vertical Retrace | video has begun vertical retrace (held set ~90 µs); the VBL interrupt source |
...
- **Vertical-retrace (VBL) interrupt:** generated by the video state machine
  ("VTIR"), at **IPL 1**. Enabled/disabled via the strobes `$00E018` (off) /
  `$00E01A` (on) (§6.1). The retrace status is **bit 2 of the Status Register**
  (`$00F800`), held set for ~90 µs after retrace begins. An emulator asserts the
  VBL once per displayed frame.
...
On success the ROM selects a boot device and loads the OS (or MacWorks). A
low-RAM byte `SYSTYPE` at logical `$2AF` records the machine class: `0` = Lisa 1,
`1` = Lisa 2, `2` = Lisa 2 + external HD, `3` = Lisa 2 + internal HD.
