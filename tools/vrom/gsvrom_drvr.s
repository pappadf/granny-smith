...
| gsvrom_equ.i so the ops macros see it in every instantiation.)

| --- The driver sBlock -------------------------------------------------------
	sBlockBegin GSDrvrBlk

GSDrvrHdr:
	dc.w	0x4C00                  | drvrFlags: dNeedLock|dStatEnable|dCtlEnable
	dc.w	0                       | drvrDelay
	dc.w	0                       | drvrEMask
	dc.w	0                       | drvrMenu
...
CtlBad:
	moveq	#controlErr,d0
	bra	DrvExit                 | unsized: DrvExit is 0 bytes ahead and
	                                | a zero-displacement bra.s can't encode
...
CtlSetEntries:
	...
	sub.w	#0x80,d0                | modes must use DirectSetEntries
