...
export async function bootWithMedia(page: Page, romRel: string, fd0Rel?: string, hd0Rel?: string, speed?: 'max' | 'realtime' | 'hardware', model?: string, vromRel?: string) {
	...
	if(vrom) params.push(`vrom=${encodeURIComponent(`${TEST_MEDIA_BASE_URL}/${vrom.baseName}`)}`);
...
export async function bootWithUploadedMedia(
	...
	await page.evaluate(async ({ hasFd, hasHd, hasVrom, hdSlot, fdWritable }) => {
		...
		// Universal → SE/30.
		const infoRaw = await ev('machine.rom.identify', ['/tmp/rom']);
		...
		if (!Array.isArray(compatible) || compatible.length === 0)
			throw new Error('machine.rom.identify: no compatible machines for /tmp/rom');
		...
		if (hasVrom) await ev('machine.vrom.load', ['/tmp/vrom']);
