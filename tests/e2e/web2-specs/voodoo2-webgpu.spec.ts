...
import { BASE_ARGS, WEBGPU_ARGS, bootWithCard, composeScript, probe, terminalRun } from '../helpers/voodoo2';
...
test('the takeover engages when the card takes the monitor, and the GPU covers exactly the walker pixels', async ({
  ...
  expect(await probe(page, 'machine.pci.slot[1].card.regs.raster')).toBe('webgpu');
  ...
  expect(text, text).toContain('draw-webgpu: takeover drawing section complete');
  ...
  expect(await probe(page, 'machine.pci.slot[1].card.regs.gpu_engaged')).toBe('true');
  ...
  expect(await probe(page, 'machine.pci.slot[1].card.video.drives_monitor')).toBe('false');
  expect(await probe(page, 'machine.pci.slot[1].card.regs.gpu_engaged')).toBe('false');
  await expect(page.locator('#screen3d')).toBeHidden();
});
