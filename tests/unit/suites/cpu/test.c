...
static const uint8_t *decode_state(const uint8_t *ptr, cpu_state_t *state) {
    ...
    state->usp = read_le32(ptr);
    ptr += 4;
    state->ssp = read_le32(ptr);
    ptr += 4;
    state->sr = (uint16_t)read_le32(ptr);
    ptr += 4;
    state->pc = read_le32(ptr);
    ptr += 4;

    // Prefetch words (stored as 32-bit values in test format)
    state->prefetch[0] = read_le32(ptr);
    ptr += 4;
    state->prefetch[1] = read_le32(ptr);
    ptr += 4;
...
static bool test_triggers_exception(const cpu_state_t *initial, const cpu_state_t *final) {
    ...
    if (final->ssp < initial->ssp && (initial->ssp - final->ssp) >= MIN_EXCEPTION_STACK_FRAME_SIZE) {
        return true;
    }
    return false;
}
...
static bool compare_cpu_state(cpu_t *cpu, const cpu_state_t *expected, const cpu_state_t *initial, char *diff_buf,
...
#define APPEND_DIFF(...)                                                                                               \
    ...
    if (actual_pc != expected_pc_adjusted) {
        match = false;
        APPEND_DIFF("  PC: 0x%08X (expected 0x%08X, was 0x%08X)\n", actual_pc, expected_pc_adjusted,
                    (initial->pc - PREFETCH_OFFSET) & 0x00FFFFFF);
    }
...
static int run_test_file(const char *filepath, test_context_t *ctx, test_stats_t *stats) {
    ...
    if (limit > MAX_TESTS_PER_FILE)
        limit = MAX_TESTS_PER_FILE;
#endif
    ...
    for (uint32_t i = 0; i < limit; i++) {
        ...
        // Compare final state
        if (!compare_cpu_state(cpu, &test.final, &test.initial, diff_buf, sizeof(diff_buf))) {
            file_failures++;
            stats->tests_failed++;
...
static int run_all_tests(test_context_t *ctx) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (!ext || ext[TEST_FILE_EXTENSION_LEN] != '\0')
            continue;
