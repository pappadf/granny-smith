...
#
# Phase 4-5 of the Copland install.  It starts from the volume
# `pm7100-copland-install` publishes — Drive-Setup-2.0d5c2-formatted with
# System 7.5.0 already on it — so it never re-pays the ~92 G instructions that
# install cost:
#
...
# that gate this one: 200 `'mach'` (a NuBus PDM), 202 16 MB of RAM,
...
# target.
#
# MEDIA: the Copland media and the volume pm7100-copland-install publishes are
# not redistributable and not part of the test data.  The test reads them from
# $GS_EXTRA_MEDIA_DIR/copland/images/ and $GS_EXTRA_MEDIA_DIR/copland/media/,
# publishes its result back to the latter, and SKIPS cleanly where they are
...
# 24 MB: inside Copland's 16-32 MB window, and past rule 202.
