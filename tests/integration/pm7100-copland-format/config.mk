...
#
# The install half of booting Copland.  It builds the volume the companion
# boot test consumes: a 230 MB Apple-branded drive, formatted
...
# image.  The split is the same reasoning as the split between install and
# boot (pm7100-copland-boot): formatting is slow, deterministic and almost
# never changes, while the two installers are what gets iterated on.  Paying
# ~5.6 G instructions of boot-and-format before every install attempt is not
# a cost worth repeating.
#
# Deliberately NOT a floppy boot: the 7.5 Disk Tools floppy carries no CD-ROM
# extensions, so Drive Setup 2.0d5c2 — which lives on the DDK CD — would be
# unreachable from it.  Apple explicitly allows installing from any
...
#
# MEDIA: the Copland media is not redistributable and not part of the test
# data.  The test reads it from $GS_EXTRA_MEDIA_DIR/copland/images/ and
# $GS_EXTRA_MEDIA_DIR/copland/media/, publishes the formatted volume back to
# the latter, and SKIPS cleanly everywhere the media is absent.  CI stays
# green without it.
#
...
# 24 MB: inside Copland's 16-32 MB window.  The script re-boots the machine
# itself so it can pin the RTC first.
