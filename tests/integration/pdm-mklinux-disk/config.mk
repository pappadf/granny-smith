...
# that.  Neither has to do the other's job.
#
# MEDIA: the DR3 disc is a 663 MB hybrid ISO that is not redistributable and
# not part of the test data.  The test reads it from
# $GS_EXTRA_MEDIA_DIR/mklinux/media/ and SKIPS cleanly without it.  The image
# it produces IS in the test data, so the per-commit boot test needs none of
# this.
