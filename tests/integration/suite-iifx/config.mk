...
# iifx-24ac, iifx-824gc (re-media'd to 7.1) and iifx-aux3-boot.
#
...
# tracked by nothing. This suite turns that into a tracked, run-not-fatal
# row.
#
# NOT covered here: paired 24-bit/32-bit rows. The addressing mode is
# PRAM-selected on a clean-ROM machine, but the selector byte is still
# unidentified — measured 2026-07-28, seeding MMFlags (PRAM $8A, pram.md
# §4.3) to $80/$00 leaves the IIfx booting 7.0.1 in 24-bit either way,
# so that byte is ruled out. The declared 32-bit IIfx cell stays in
# matrix-targets.json marked `blocked` so the debt keeps reporting;
# 32-bit coverage meanwhile comes from the IIci and Q950 7.6 rows, where
# the System selects it itself.
#
